...
export function initMarketing() {
  ...
  document.getElementById('docs-page')!.innerHTML = `
    ...
    <section id="quick-start"><h2>Quick start</h2><ol><li><a href="/dashboard" data-route="/dashboard">Launch the dashboard</a> and select <strong>Run Analysis</strong>.</li><li>Enter a wallet address, select one of 6 EVM networks (Base, Ethereum, BNB Smart Chain, Arbitrum One, Polygon, Optimism), and choose a 7, 30, or 90 day timeframe.</li><li>Run the analysis. Review source status before interpreting balances, cash flow, or risk.</li><li>Explore assets and risk, then open AI Copilot & Workflow to ask report-specific questions or copy a review draft.</li></ol><h3>Run locally</h3><pre><code>npm ci
npm run build
npm start
# Open http://127.0.0.1:3000</code></pre><p>Requires Node.js 22 or newer. Configure environment values using the repository’s .env.example file.</p></section>
    <section id="networks"><h2>Networks & tracked assets</h2><div class="table-wrap"><table><thead><tr><th>Network</th><th>Chain ID</th><th>Native asset</th><th>Selected tokens</th></tr></thead><tbody><tr><td>Base</td><td>8453</td><td>ETH</td><td>USDC, WETH, cbBTC</td></tr><tr><td>Ethereum</td><td>1</td><td>ETH</td><td>USDC, USDT, WETH, WBTC</td></tr><tr><td>BNB Smart Chain</td><td>56</td><td>BNB</td><td>USDT, USDC, WBNB</td></tr><tr><td>Arbitrum One</td><td>42161</td><td>ETH</td><td>USDC, USDT, ARB</td></tr><tr><td>Polygon</td><td>137</td><td>POL</td><td>USDC, USDT, WMATIC</td></tr><tr><td>Optimism</td><td>10</td><td>ETH</td><td>USDC, USDT, OP</td></tr></tbody></table></div><p>Tickers identify assets, not networks: Base uses ETH for gas. WETH, WBNB, and WMATIC are wrapped assets; cbBTC and WBTC are Bitcoin wrappers. LedgerMind reads configured token lists across 6 major EVM chains.</p></section>
    ...
    <section id="workflow"><h2>Reports, copilot & approval</h2><p>Report History loads saved reports. Each report includes an observation time, source statuses, report hash, and a Markdown export. The copilot answers from the selected report powered by zero-config Groq Free API (llama-3.3-70b-versatile) or configurable providers.</p><p>Workflow drafts support team review. Creating or copying a draft does not submit a multisig transaction or execute a trade. The local SHA-256 audit chain helps detect modifications against a trusted checkpoint; it is not external notarization.</p></section>
    <section id="x402"><h2>Premium data &amp; x402</h2><p>Open the standalone <strong>x402 Premium Market</strong> page (/premium), connect your paying wallet with RainbowKit and sign the login challenge to prove ownership, then select Get premium quote. Login does not authorize payment. Purchase history and reports are private to the authenticated owner; global purchases are still filtered by the paying wallet. The app checks USDC balance and merchant terms on Base. Review the 0.01 USDC price, recipient and expiry before selecting Confirm &amp; pay.</p><p>Your wallet signs a single-use USDC authorization. A successful merchant settlement unlocks Top 10 Whale Wallet Tracking, AI Portfolio Strategy Recommendations, 24h Range Bar, and Liquidation Risk Heatmaps.</p><pre><code>BROWSER_PREMIUM_ENABLED=true
...
}</code></pre><p>Replace 0x… with a valid 42-character EVM address and chainId with 8453 (Base), 1 (Ethereum), 56 (BSC), 42161 (Arbitrum), 137 (Polygon), or 10 (Optimism). The response includes assets, transactions, metrics, risks, dataSources, brief, workflowDraft, and report metadata.</p><div class="table-wrap"><table><thead><tr><th>Endpoint</th><th>Purpose</th></tr></thead><tbody><tr><td>GET /api/reports</td><td>Saved report list</td></tr><tr><td>GET /api/reports/:id</td><td>Read one report</td></tr><tr><td>GET /api/reports/:id/markdown</td><td>Export report</td></tr><tr><td>GET /api/reports/:id/audit</td><td>Report audit events</td></tr><tr><td>POST /api/agent/ask</td><td>Ask with reportId and question</td></tr><tr><td>POST /api/workflows/draft</td><td>Retrieve a draft using reportId</td></tr><tr><td>GET /api/tickers</td><td>BTC, ETH, BNB quotes in USDT</td></tr><tr><td>POST /api/premium/quote</td><td>Generate x402 merchant quote</td></tr><tr><td>POST /api/premium/settle</td><td>Verify settlement & unlock premium market data</td></tr><tr><td>GET /api/premium/purchases</td><td>List saved x402 premium purchases</td></tr></tbody></table></div></section>
