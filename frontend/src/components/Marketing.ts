...
# Open http://127.0.0.1:3000</code></pre><p>Requires Node.js 24 or newer. Configure environment values using the repository’s .env.example and .env.live.example files.</p></section>
    ...
    <section id="x402"><h2>Premium data & x402</h2><p>x402 is used for paid data access. Standard public-chain analysis works without premium access. Premium requests need provider configuration, and live spending needs explicit approval plus enabled payment configuration and budget checks.</p><pre><code>PAYMENT_MODE=binance
REAL_PAYMENTS_ENABLED=true
BAW_CLI_JS=/absolute/path/to/agentic-wallet/dist/index.js</code></pre><p>Enable these only when intentionally configuring real paid access. “Not configured” or “approval required” is not a payment receipt. Inspect the returned premium status and audit evidence.</p></section>
