...
# Open http://127.0.0.1:3000</code></pre><p>Requires Node.js 22 or newer. Configure environment values using the repository’s .env.example file.</p></section>
    ...
    <section id="x402"><h2>Premium data &amp; x402</h2><p>Open the standalone <strong>x402 Premium Market</strong> page (/premium), connect your paying wallet with RainbowKit, then select Get premium quote. The app checks USDC balance and merchant terms on Base. Review the 0.01 USDC price, recipient and expiry before selecting Confirm &amp; pay.</p><p>Your wallet signs a single-use USDC authorization. A successful merchant settlement unlocks Top 10 Whale Wallet Tracking, AI Portfolio Strategy Recommendations, 24h Range Bar, and Liquidation Risk Heatmaps.</p><pre><code>BROWSER_PREMIUM_ENABLED=true
